  ...
  for input_key, channel in tfx_node.inputs.items():
    if compiler_utils.is_resolver(tfx_node):
      ...
      if (
          ...
          and channel.min_count is not None
          and ((channel.min_count == 0) != spec_param.allow_empty)
      ):
        raise ValueError(
            f'Node {tfx_node.id} input channel {input_key} allow_empty is set'
            f' to {spec_param.allow_empty} but the provided channel has a'
            f' min_count of {channel.min_count}. If the component spec'
            ' explicitly sets allow_empty, then the channel must match.'
        )
      elif channel.min_count is None:
        min_count = 0 if spec_param.allow_empty else 1
      else:
        min_count = channel.min_count
    else:
      min_count = 1
